...
class CarState(CarStateBase, MadsCarState, SnGCarState):
  ...
  def update(self, can_parsers) -> tuple[structs.CarState, structs.CarStateSP]:
    ...
    # BluePilot: angle-LKAS cars use ES_Status, which correctly drops when braking at a stop.
    if self.CP.flags & SubaruFlags.LKAS_ANGLE:
      ...
      # ES_Status is missing on hybrid, so retain the existing ES_Brake source.
